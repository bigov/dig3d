...
class glfw_cover
{
  public:
    ...
    glfw_window& create_window(const char* row);
    void delete_window(glfw_window& w);

  private:
    static bool gl_is_loaded;
    static interface_gl_context* error_observer;
    std::vector<glfw_window> AppWindows {};
...
class glfw_window : public glfw_cover
{
  public:
...
class dig3dapp: public interface_gl_context
{
  public:
    dig3dapp(std::string Title);
    ~dig3dapp(void) {};
    virtual void event_keyboard(int _key, int _scancode, int _action, int _mods);

  private:
    glfw_cover GlfwCover {};
    glfw_window* MainWin = nullptr;
};
